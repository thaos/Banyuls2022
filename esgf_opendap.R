...
# esgf_search <- xmlParse(file = "esg-dn1.nsc.liu.se.xml")
...
get_opendap_url <- function(doc){
  data_node <- xml_child(doc, search = which(xml_attr(xml_children(doc), attr = "name") == "data_node")) |> xml_text()
  url_thredd <-  xml_child(doc, search = which(xml_attr(xml_children(doc), attr = "name") == "url")) |> xml_text()
  url_thredd <- "cordex.output.EUR-11.ICTP.MOHC-HadGEM2-ES.historical.r1i1p1.RegCM4-6.v1.day.huss.v20190502.xml"
  thredd <- read_xml(url_thredd) |> xml_ns_strip()
  opendap <- xml_child(thredd, search = which(xml_attr(xml_children(thredd), attr = "serviceType") == "OpenDAP"))
  ...
  opendap_urls <- opendap_urls[xml_attr(opendap_urls, "serviceName") == "OpenDAPServer"] |>
...
load_data_from_opendap <- function (opendap_urls, varname, icoords){
